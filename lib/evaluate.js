...
export default function evaluate(webpackActions, webpackFormats, webpackRC, target, ...config) {
  ...
  const parsedRootFormats = parseFormats(formats, webpackRC.formats);
  const parsedFormats = parseFormats(formats, parsedEnv.formats);
...
function parseFormats(formatDefinition, formats) {
  if(!formats) {
    return;
  }

  return formats.map((format) => {
    return formatDefinition[format];
  });
}
